      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredItems.length === 0 ? (
          <div className="col-span-full bg-white/[0.02] border border-white/5 rounded-3xl p-12 text-center flex flex-col items-center justify-center backdrop-blur-md">
            ...
            <h3 className="text-xl font-display font-bold text-white mb-2">Inventaris Kosong</h3>
            <p className="text-white/40">Belum ada barang yang ditambahkan ke inventaris.</p>
          </div>
        ) : (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                  {canUpdate && (
                    <button onClick={() => handleOpenModal(true, item)} className="p-2 text-white/50 hover:text-white"><Edit2 className="w-4 h-4" /></button>
                  )}
                  {canDelete && (
                    <button onClick={() => handleDelete(item.id)} className="p-2 text-white/50 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                  )}
                </div>
              </div>

              <div className="mb-4">
                <h3 className="text-xl font-display font-black text-white mb-1 line-clamp-1">{item.name}</h3>
              ...
              <div className="space-y-3 mt-auto">
                ...
                <div className="flex items-center justify-between text-xs text-white/50">
                  ...
                  <div className="font-bold text-white bg-white/10 px-2 py-0.5 rounded">Qty: {item.quantity}</div>
                </div>
                {item.location && (
                  <div className="flex items-center gap-3 text-xs text-white/50">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5 relative">
                ...
                <div className="col-span-full">
                  <label className="block text-xs font-bold uppercase tracking-widest text-white/50 mb-2">Nama Barang</label>
                  <input name="name" type="text" required defaultValue={modal.data?.name} placeholder="Misal: Proyektor Epson X500" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-primary/50 focus:bg-white/10 transition-colors" />
                </div>
                
                <div>
                  <label className="block text-xs font-bold uppercase tracking-widest text-white/50 mb-2">Kode Barang (Opsional)</label>
                  <input name="code" type="text" defaultValue={modal.data?.code || ""} placeholder="SRE-INV-001" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-primary/50 focus:bg-white/10 transition-colors font-mono" />
                </div>

                <div>
                  <label className="block text-xs font-bold uppercase tracking-widest text-white/50 mb-2">Jumlah / Kuantitas</label>
                  <input name="quantity" type="number" min="1" required defaultValue={modal.data?.quantity || 1} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-primary/50 focus:bg-white/10 transition-colors" />
                </div>

                <div className="relative z-[50]">
                  <label className="block text-xs font-bold uppercase tracking-widest text-white/50 mb-2">Kategori</label>
                  <CustomSelect 
                    name="category" 
                ...
                <div className="relative z-[49]">
                  <label className="block text-xs font-bold uppercase tracking-widest text-white/50 mb-2">Kondisi Fisik</label>
                  <CustomSelect 
                    name="condition" 
                ...
                <div className="col-span-full">
                  <label className="block text-xs font-bold uppercase tracking-widest text-white/50 mb-2">Lokasi Penyimpanan</label>
                  <input name="location" type="text" defaultValue={modal.data?.location || ""} placeholder="Loker Sekretariat SRE" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-primary/50 focus:bg-white/10 transition-colors" />
                </div>
                ...
                <div className="col-span-full flex items-center gap-3 mt-2">
                  ...
                  />
                  <label htmlFor="isAvailable" className="text-sm font-medium text-white cursor-pointer">Barang Tersedia (Bisa Dipinjam)</label>
                </div>

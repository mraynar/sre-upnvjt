...
export default function ActivityPublicClient({ activities }) {
  ...
  const featuredActivity = displayActivities[0];
  ...
  return (
    <div className="min-h-screen bg-[#07130e] text-white pt-32 pb-0 selection:bg-[#e8ecc4] selection:text-[#07130e]">
      ...
      <section className="px-6 md:px-12 lg:px-20 max-w-[1400px] mx-auto mb-32">
        ...
        >
          <img 
            src={featuredActivity.imageUrl || featuredActivity.img || "https://images.unsplash.com/photo-1497435334941-8c899ee9e8e9?q=80&w=2000&auto=format&fit=crop"} 
          ...
          <div className="absolute bottom-8 left-8 right-8 md:bottom-16 md:left-16 md:right-16 flex flex-col md:flex-row justify-between items-end gap-6">
            <div className="flex flex-col gap-4 max-w-2xl">
              <span className="inline-block px-4 py-1.5 rounded-full border border-[#e8ecc4]/40 text-[#e8ecc4] text-[10px] md:text-[12px] font-bold uppercase tracking-widest backdrop-blur-md w-fit">
                Featured Activity • {new Date(featuredActivity.createdAt || featuredActivity.date).getFullYear() || "2026"}
              </span>
      ...
      <section className="bg-[#e8ecc4] text-[#07130e] pt-32 pb-40 rounded-t-[40px] relative z-10">
        <div className="px-6 md:px-12 lg:px-20 max-w-[1400px] mx-auto">
          ...
          <div className="flex flex-col">
            {displayActivities.slice(1).map((item, index) => (
              <motion.div 
                key={item.id}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div className="lg:w-1/4 flex flex-col justify-between">
                  ...
                  <div>
                    ...
                    <span className="text-[14px] font-bold uppercase tracking-wide text-[#07130e]">
                      {item.createdAt ? new Date(item.createdAt).toLocaleDateString("id-ID", { month: "long", year: "numeric" }) : item.date}
                    </span>
                  </div>
                </div>
                ...
                <div className="lg:w-1/2 flex flex-col justify-center">
                  <span className="text-[11px] font-mono uppercase tracking-[0.2em] text-[#07130e]/50 block mb-3">
                    {item.category || "General Activity"}
                  </span>
                ...
                <div className="lg:w-1/4 flex items-center justify-end">
                  <div className="w-full lg:w-48 aspect-video lg:aspect-[4/5] overflow-hidden rounded-2xl relative bg-black/10">
                    {item.imageUrl || item.img ? (
                      ...
                      />
                       ...
                       <div className="w-full h-full flex items-center justify-center text-[#07130e]/20 text-[10px]">No Image</div>
                    )}
                  </div>
                </div>
